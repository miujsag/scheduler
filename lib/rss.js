...
async function run(elasticSearchClient) {
  try {
    ...
    await Promise.all(
      sites.map((site) =>
        processArticles(site, categories, elasticSearchClient)
      )
    );
...
function processArticles(site, categories, elasticSearchClient) {
  return Scraper.RSS.scrape(site.feed)
    ...
    .then((articles) =>
      scrapeArticles(articles, site, categories, elasticSearchClient)
    );
}
...
async function scrapeArticles(articles, site, categories, elasticSearchClient) {
