...
async function run () {
  try {
    ...
    logError(new Error(createLogMessage({
...
async function scrapeArticles (articles, site, categories) {
  ...
  try {
    for (const article of newArticles) {
      const scrapedArticle = await Scraper.Article.scrape(article.url, site.article_selector)

      if (scrapedArticle && scrapedArticle.html) {
        const textCandidate = scrapedArticle.content || scrapedArticle.description || scrapedArticle.title
        const category = Classifier.Category.extractFromContent(
          Classifier.Category.extractFromCandidates(null, scrapedArticle.category, categories),
          textCandidate, categories
        )
    ...
    throw new Error(createLogMessage({
